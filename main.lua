        ...
        for y = 0, gridinfo.sizey - 1 do
            ...
            for x = 0, gridinfo.sizex - 1 do
                local spotinfo = {
                    ...
                    y = y + 1,
                    selected = false,
                    debris = (math.random(1, 10) == 10 and "ice") or (math.random(1, 10) == 10 and "junk"),
                    near = 0,
                    state = "covered",
                }
...
function love.draw()
    ...
    elseif inmenu == "pause" then
        love.graphics.draw(overlay)
        local ox = love.graphics.newText(font, "Press ESC to unpause"):getWidth()/2
        love.graphics.print("Press ESC to unpause", font, 338*0.5, 258*0.4, 0, 1.5, 1.5, ox)
    elseif inmenu == "main" then
        love.graphics.draw(overlay)
    elseif inmenu == "lose" then
    end
end
	...
	if button == 1 then
		...
		while #stack > 0 do
			...
			if gridinfo.grid[y] and gridinfo.grid[y][x] then
				...
				if gridinfo.grid[y][x].debris == "ice" then
					clicks = clicks + 6
